...
from leetcode.utils.constants import REPORTS_DIR


df = get_clean_records_for_india()
min_date, max_date = df["date"].min().replace("/", "_"), df["date"].max().replace("/", "_")

with open(f"{REPORTS_DIR}/report_{min_date}_to_{max_date}_dark.md", "w") as f:
    f.write("## Notes")
    ...
    sample_df = df[["post_title", "href", "date", "company", "title", "yoe", "salary", "location", "post"]].sort_values(
    ...
    for r in sample_df.iterrows():
        ...
        f.write(f'salary : `₹ {int(r["salary"])}`<br>')
